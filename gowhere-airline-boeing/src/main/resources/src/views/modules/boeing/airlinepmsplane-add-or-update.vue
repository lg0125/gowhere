    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.planeId) {
            this.$http({
              url: this.$http.adornUrl(`/boeing/airlinepmsplane/info/${this.dataForm.planeId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.size = data.airlinePmsPlane.size
                this.dataForm.status = data.airlinePmsPlane.status
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/boeing/airlinepmsplane/${!this.dataForm.planeId ? 'save' : 'update'}`),
              method: 'post',
